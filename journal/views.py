...
@login_required(login_url='ny-login')
...
def update_thought(request, pk):
    ...
    try:
        # checking that user also matches thought ID
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Changed your thought!")
            return redirect('my-thoughts')
...
def delete_thought(request, pk):
    ...
    if request.method == 'POST':
        thought.delete()
        messages.success(request, "Thought forgotten!")
        return redirect('my-thoughts')
